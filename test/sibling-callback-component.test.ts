...
describe("sibling callback temp components", () => {
  ...
  it.runIf(process.platform === "win32").each(["\\\\?\\C:\\", "\\\\.\\C:\\"])(
    "preserves the namespace-root parent %s without admitting aliases", (namespaceRoot) => {
      expect(resolveCallbackTempPath(namespaceRoot, "stage.part")).toBe(`${namespaceRoot}stage.part`);
      for (const invalidParent of [namespaceRoot.slice(0, -1), `${namespaceRoot}bad:stream\\..\\parent`]) {
        expect(() => resolveCallbackTempPath(invalidParent, "stage.part"))
          .toThrow(expect.objectContaining({ code: "invalid-path", details: { reason: "windows-path-alias" } }));
      }
    },
  );
