...
describe("Root compatibility destination resolution", () => {
  it.each([true, false])("admits case-folded canonical Root spelling only with the retained identity (same Root=%s)", async sameRoot => {
    const platform = Object.getOwnPropertyDescriptor(process, "platform")!;
    const windowsRelative = path.win32.relative;
    const windowsJoin = path.win32.join;
    Object.defineProperty(process, "platform", { value: "win32" });
    try {
      // Keep Windows path semantics when this deterministic identity test runs
      // on a POSIX host; all filesystem observations below are supplied here.
      vi.spyOn(path, "relative").mockImplementation(windowsRelative);
      vi.spyOn(path, "join").mockImplementation(windowsJoin);
      const rootPath = "C:\\workspace\\root";
      const targetPath = `${rootPath}\\target`;
      const candidateRoot = "C:\\workspace\\ROOT";
      vi.spyOn(realpathSync, "native").mockReturnValue(`${candidateRoot}\\target`);
      const lstat = vi.spyOn(fsSync, "lstatSync").mockImplementation(((candidate, options) => {
        expect(candidate).toBe(candidateRoot);
        expect(options).toEqual({ bigint: true });
        return {
          dev: 7n, ino: sameRoot ? 11n : 12n,
          isSymbolicLink: () => false, isDirectory: () => true,
        } as fsSync.BigIntStats;
      }) as typeof fsSync.lstatSync);
      const run = vi.fn();
      const result = withRootFallbackCompatibilityLock({
        rootPath, targetPath, rootIdentity: { dev: 7n, ino: 11n },
      }, run);
      if (sameRoot) {
        await expect(result).resolves.toBeUndefined();
        expect(run).toHaveBeenCalledWith(expect.objectContaining({ targetPath, relativePath: "target" }));
      } else {
        await expect(result).rejects.toMatchObject({ code: "path-alias" });
