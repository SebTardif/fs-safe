...
function noReplaceAdapter(rootPath: string, onRename?: () => void, onOpen?: (relativePath: string) => void) {
...
it("snapshots overwrite before awaited move admission", async () => {
  const directory = await tempRoot("fs-safe-root-move-overwrite-snapshot-");
  const source = path.join(directory, "source");
  const target = path.join(directory, "target");
  await fs.writeFile(source, "source");
  await fs.writeFile(target, "existing target");
  ...
  await expect(scoped.move("source", "target", options))
    .rejects.toMatchObject({ code: "already-exists" });
  expect(changed).toBe(true);
  expect(await fs.readFile(source, "utf8")).toBe("source");
  expect(await fs.readFile(target, "utf8")).toBe("existing target");
});
...
it.skipIf(process.platform === "win32").each(["source", "target"] as const)(
  "reauthorizes a denied canonical %s selected during native parent admission", async boundary => {
    const directory = await tempRoot("fs-safe-root-move-denied-parent-");
    ...
    await Promise.all([fs.mkdir(allowed), fs.mkdir(denied), fs.mkdir(incoming)]);
    await fs.writeFile(path.join(allowed, "source"), "original allowed source");
    await fs.writeFile(path.join(denied, "source"), "protected source");
    ...
    const adapter = noReplaceAdapter(directory, undefined, relativePath => {
      if (redirected || relativePath !== "allowed") return;
      fsSync.renameSync(allowed, path.join(directory, "parked-allowed"));
      // POSIX openBeneath permits contained ordinary symlinks; the denied
      // directory itself never moves or changes its protected pathname.
      fsSync.symlinkSync("denied", allowed, "dir");
      redirected = true;
    });
    __setNativeLoaderForTest(() => adapter.binding);
    configureFsSafeNative({ mode: "require" });
    const scoped = await root(directory, { denyMutations: { prefixes: [denied] } });
    ...
    expect(adapter.renameNoReplace).not.toHaveBeenCalled();
    expect(await fs.readFile(path.join(denied, "source"), "utf8")).toBe("protected source");
    expect(await fs.readFile(path.join(incoming, "source"), "utf8")).toBe("incoming source");
    await expect(fs.lstat(path.join(denied, "published"))).rejects.toMatchObject({ code: "ENOENT" });
