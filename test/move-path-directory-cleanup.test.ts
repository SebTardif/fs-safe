...
describe("copied-source directory cleanup", () => {
  describe.each([false, true])("nested=%s", (nested) => {
    it.each(["directory", "file", "missing"] as const)(
      "reports a %s replacement during child cleanup as stale", async (replacement) => {
        ...
        // Each admitted directory checks authority before its final observation.
        // A surviving outer directory may still attempt its own empty removal.
        expect(mutation).toHaveBeenCalledTimes(mutationCountAtReplacement + (nested ? 2 : 1));
        expect(published).toHaveBeenCalledTimes(1);
  ...
  it("checks mutation authority before the final directory observation", async () => {
    const move = await fixture();
    ...
    await expect(movePathWithCopyFallback({
      ...
      assertBeforeMutation: () => {
        if (childRemoved) throw denied;
      },
    })).rejects.toBe(denied);

    expect(finalObservation).toBe(false);
    expect(rmdir).not.toHaveBeenCalled();
    await expect(fs.readdir(move.source)).resolves.toEqual([]);
    await expect(fs.readFile(move.publishedChild, "utf8")).resolves.toBe("copied");
  });
  ...
  it("preserves a directory replaced by its final mutation-authority callback", async () => {
    const move = await fixture();
    let childRemoved = false;
    let replaced = false;
    afterChildUnlink(move.child, () => { childRemoved = true; });
    ...
    await expect(movePathWithCopyFallback({
      ...
      sourceHardlinks: "reject",
      assertBeforeMutation: () => {
        if (!childRemoved || replaced) return;
        fsSync.renameSync(move.directory, move.parked);
        fsSync.mkdirSync(move.directory);
        replaced = true;
      },
    })).rejects.toMatchObject({ code: "ESTALE" });
