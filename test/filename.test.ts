...
describe("sanitizeUntrustedFileName", () => {
  ...
  it.each([
    ...
    ["Windows punctuation", '<>:"|?*'],
    ["path separators", "/\\/\\"],
    ["discarded characters and whitespace", " \t<>:\r\n "],
    ["overlength discarded input", "<>".repeat(160)],
  ])("rejects a fully discarded %s candidate in both positions", (_label, candidate) => {
    expect(sanitizeAtPosition("primary", candidate)).toBe("fallback.bin");
    expect(sanitizeAtPosition("fallback", candidate)).toBe("file");
  });

  it.each([
    ["retained beginning", "a<>", "a"],
    ["retained middle", "<>a<>", "a"],
    ["retained end", "<>a", "a"],
    ["dot", "<>.txt<>", ".txt"],
    ["ordinary Unicode", "<>é<>", "é"],
    ["Mongolian vowel separator", "<>\u180e<>", "\u180e"],
    ["zero-width space", "<>\u200b<>", "\u200b"],
  ])("preserves a %s near-miss in both positions", (_label, candidate, expected) => {
