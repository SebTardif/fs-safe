...
export function resolveSecureTempRoot(options: ResolveSecureTempRootOptions): string {
  ...
  const accessSync = suppliedAccess ?? (process.platform === "win32"
    ? (candidate: string, mode?: number) => hostAccess(pathForWindowsFilesystem(candidate), mode)
    : hostAccess);
  ...
  const mkdirSync = suppliedMkdir ?? (
    suppliedLstat !== undefined || suppliedAccess !== undefined || suppliedDescriptor !== undefined
      ...
      : (directory: string, mkdirOptions: { recursive: boolean; mode?: number }) =>
        hostMkdir(recursiveMkdirPath(pathForWindowsFilesystem(directory)), mkdirOptions)
  );
  ...
  const fallback = (): string => {
    const base = tmpdir();
    assertNoWindowsPathAlias(
      base, "filesystem", "system temp directory uses a Windows filesystem namespace alias", admissionPlatform,
    );
    ...
    assertNoWindowsPathAlias(
      fallbackPath, "filesystem", "fallback temp directory uses a Windows filesystem namespace alias", admissionPlatform,
    );
    return fallbackPath;
  };
